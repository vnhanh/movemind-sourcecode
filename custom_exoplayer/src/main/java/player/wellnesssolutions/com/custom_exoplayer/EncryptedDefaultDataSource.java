...
public class EncryptedDefaultDataSource implements DataSource {
    ...
    public EncryptedDefaultDataSource(String key, Context context, DataSource baseDataSource) {
        this.key = key;
    ...
    public long open(DataSpec dataSpec) throws IOException {
        ...
        if (Util.isLocalFileUri(dataSpec.uri)) {
            dataSource = getEncryptedDataSource(key, getFileDataSource());
        } else {
            dataSource = getEncryptedDataSource(key, baseDataSource);
        }
    ...
    private DataSource getEncryptedDataSource(String key, DataSource upstreamDataSource) {
        if (aesCipherDataSource == null) {
            aesCipherDataSource = new AesCipherDataSource(Util.getUtf8Bytes(key), upstreamDataSource);
