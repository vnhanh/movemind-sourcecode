...
object RealmObjectsToDVideosMapper {
    fun mapList(source: RealmResults<RealmDVideo>): ArrayList<MMVideo> {
        val res = ArrayList<MMVideo>()

        for (ele: RealmDVideo in source) {
            res.add(mapObject(ele))
        }
        return res
    }

    private fun mapObject(source: RealmDVideo): MMVideo {
