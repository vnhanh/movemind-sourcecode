...
object VideosToRealmObjectsMapper {
    fun mapList(source: ArrayList<MMVideo>, tag: String): RealmList<VideoEntity> {
        val res = RealmList<VideoEntity>()
    ...
    private fun mapObject(source: MMVideo, groupTag: String): VideoEntity {
        val res = VideoEntity().apply {
            id = source.id
            tag = groupTag
        ...
        source.languages?.also {
            ...
            for (ele: MMVideoLanguage in it) {
                languages.add(RealmLanguage().also {
                    it.id = ele.id
                    it.name = ele.name
                })
            }
