...
object HandleVideosOnceStopCasting {
    ...
    fun handlePlayingVideos(activity: Activity?, handler: Handler, schedulePresenter: IScheduleContract.Presenter?) {
        handler.post(object: Runnable{
            override fun run() {
                activity?.also { activity ->
                    PreferenceHelper.getInstance(activity).getInt(ConstantPreference.PRESENTATION_PLAYED_MODE, PlayMode.UNKNOWN.value).also { modeCasting ->
